...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (q && !('error' in q)) {
        ...
        setQuestionTextZh(q.questionTextZh ?? '')
        setOptions(q.options.length >= 2 ? q.options : [...q.options, ''])
        setCorrectAnswer(q.correctAnswer)
        setExplanation(q.explanation)
        setExplanationZh(q.explanationZh ?? '')
        setDomain(q.domain as Domain)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-foreground mb-6">{t('questionForm.editTitle')}</h1>
